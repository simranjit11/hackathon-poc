...
**As a** developer  
**I want to** have database schema for banking entities (accounts, loans, transactions, alerts, notifications)  
...
1. **Database Models**: Prisma schema includes all banking models:
   ...
   - `PaymentAlert` model with fields: id, userId, alertType, threshold, accountId, isActive, timestamps
   - `NotificationPreference` model with fields: id, userId, channel, eventType, isEnabled, timestamps
...
2. **Relationships**: All models have proper foreign key relationships:
   ...
   - PaymentAlert → User (many-to-one)
   - NotificationPreference → User (many-to-one)
   - Beneficiary → Transaction (one-to-many, add to existing Beneficiary model)

3. **Indexes**: Appropriate indexes created for:
   - Foreign keys (userId, accountId, beneficiaryId)
   - Frequently queried fields (accountType, status, transactionType, paymentSessionId)
...
6. **Seed Data**: Seed script includes sample banking data:
   ...
   - Sample payment alerts and notification preferences
...
- [ ] **Task 1: Prisma Schema Updates**
  ...
  - [ ] Add `PaymentAlert` model to `prisma/schema.prisma`
  - [ ] Add `NotificationPreference` model to `prisma/schema.prisma`
  - [ ] Update `User` model with new relations
  - [ ] Update `Beneficiary` model with `transactions` relation
...
- Sample alerts and notification preferences
